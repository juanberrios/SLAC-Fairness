  ...
  implementUMS <- function(x, UMS) {
    ...
    if (startsWith(UMS, "1")) {
      ...
      if (UMS == "1.1") {
        ...
        set.seed(seed)
        x <- x %>% 
          ##Downsample men's data so there's a Gender balance
          group_by({{group}}) %>% 
    ...
    if (startsWith(UMS, "2")) {
      if (startsWith(UMS, "2.1")) {
        ...
        if (fileStem == "0.1.1") {
          ##For UMS 2.1.[123], rank vars by importance (in predicting Gender)
          impVars <- impVars %>% 
        ...
        ##  in UMS 0.1.1 (classifier of Gender)
        x <- x %>% 
...
##Specify model formula based on UMS (e.g., Rpresent ~ . - Gender)
...
summaryGender <- function(data, lev=NULL, model=NULL, obsCol="obs", 
  ...
  preds <- with(data, prediction(Male, obs))
  auc <-
    preds %>%
    performance(measure="auc") %>%
    attr("y.values") %>%
  ...
  data <- data %>%
    mutate(PredCutoff = if_else(Male >= cutoff, "Male", "Female"),
           PredHalf = if_else(Male >= 0.5, "Male", "Female"),
           AccCutoff = PredCutoff==obs,
           AccHalf = PredHalf==obs)
...
printData <- function(data, 
                      ...
                      group=Gender,
                      outlierCols=FALSE, 
                      excludeCols=c(Rpresent, Gender, contains("Outlier")),
                      umsTxt="../Input-Data/UMS-List.txt") {
  ##Validate args
  umsValidate(strategy, umsTxt=umsTxt)
  stopIfNoCol(data, {{dependent}}, "dependent", "data")
